...
backend_default<dal::detail::data_parallel_policy, Descriptor, Graph>::operator()(
    ...
    return dal::backend::dispatch_by_device(
        ...
        [&]() {
            // GPU path: build topology, extract edge values, delegate to GPU kernel
            ...
            auto& queue = gpu_ctx.get_queue();

            // Transfer host topology to device memory
            auto device_topo =
                dal::preview::detail::topology_to_device<std::int32_t>(queue, t);

            // Create a lightweight GPU view from the device topology
            const auto gpu_view = backend::make_gpu_view(device_topo);

            return backend::run_shortest_paths_gpu<float_t, task_t, value_type, std::int32_t>(
                gpu_ctx,
                descriptor,
                gpu_view,
                vals);
...
// Explicit instantiations for supported descriptor/graph combinations
// int32 edge weights
using default_descriptor_int32 = descriptor<
    float,
...
using graph_int32 = dal::preview::directed_adjacency_vector_graph<std::int32_t, std::int32_t>;
using graph_double = dal::preview::directed_adjacency_vector_graph<std::int32_t, double>;

template struct ONEDAL_EXPORT backend_default<dal::detail::data_parallel_policy,
                                              default_descriptor_int32,
                                              graph_int32>;
