...
// Single triangle: 3 vertices, all connected, 1 component
...
// Two disconnected edges: 4 vertices, 2 components
...
// Complete graph K5: all vertices connected, 1 component
...
public:
    ...
    std::array<std::int32_t, 1> cols = { 0 }; // dummy, not used
    std::array<std::int64_t, 6> rows = { 0, 0, 0, 0, 0, 0 };
};

// Mixed: triangle + edge + isolated vertex = 3 components
...
// Line graph: 0-1-2-3-4 = 1 component
...
public:
    ...
    /// Validates that the component labeling is consistent:
    /// - The number of unique labels matches the expected component count
    /// - Vertices connected by an edge share the same label
    template <typename GraphType>
    void check_vertex_partitioning() {
        ...
        // Verify number of unique labels matches component count
        ...
        // Verify connected vertices have the same label
