...
backend_default<dal::detail::data_parallel_policy, Descriptor, Graph>::operator()(
    ...
    return dal::backend::dispatch_by_device(
        ...
        [&]() {
            // GPU path: build topology and delegate to GPU kernel
            using topology_type =
            ...
            return backend::vertex_partitioning_kernel_gpu<float_t, task_t, topology_type>()(
                gpu_ctx,
                descriptor,
                t);
        });
...
using default_graph = dal::preview::undirected_adjacency_vector_graph<>;
